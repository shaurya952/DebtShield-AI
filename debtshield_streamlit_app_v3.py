...
def debtshield_chatbot(question, row, selected, df=None):
    ...
    if not q:
        return "Ask me about this county's risk level, strongest drivers, or recommended interventions."

    if df is not None and "compare" in q and " and " in q:
    ...
    if any(w in q for w in ["hello", "hi", "hey"]):
        return f"Hello! I can explain **{selected}** — its risk level, drivers, and recommended actions."
    ...
    if any(w in q for w in ["why", "driver", "cause", "highest"]):
    ...
    if any(w in q for w in ["recommend", "action", "solution", "priority", "help"]):
        items = recs(row)
        actions = " ".join([f"**{t}:** {x}" for t, x in items])
        return f"Priority: **{driver}**. {actions}" + disclaimer

    for key in ["housing", "debt", "cost", "energy", "food"]:
        if key in q:
            label = {"housing": "Housing", "debt": "Debt", "cost": "Cost of Living", "energy": "Energy", "food": "Food Access"}[key]
            return f"{label} stress for {selected} is **{scores[label]:.1f}**." + disclaimer

    return (f"For **{selected}**, the main issue is **{driver} ({driver_score:.1f})**, overall risk **{risk}**. "
            f"Try: \u201cWhy is this county at risk?\u201d, \u201ccompare {selected} and [another county]\u201d, or \u201cGive me a short summary.\u201d" + disclaimer)
