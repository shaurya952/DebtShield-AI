...
# ============================================================
# DebtShield AI v9 — final polished app version with profile assistant
# ============================================================
...
.hero {
    position: relative;
    overflow: hidden;
    padding: 1.35rem 1.65rem 1.55rem 1.65rem;
    border-radius: 30px;
    background:
      radial-gradient(circle at 12% 18%, rgba(59,130,246,.17), transparent 27%),
    ...
    box-shadow: 0 22px 46px rgba(59,130,246,.19);
...
.hero::after {
    ...
    opacity: .32;
    background:
      repeating-radial-gradient(ellipse at center, rgba(37,99,235,.35) 0px, rgba(37,99,235,.35) 1.4px, transparent 3px, transparent 10px);
    border-radius: 50%;
...
.badge {
    ...
    margin-right:.38rem;
    margin-bottom:.62rem;
    box-shadow: 0 4px 11px rgba(37,99,235,.13);
}
.hero-main {
    display:flex;
    align-items:center;
...
.kpi-card {
    ...
    box-shadow: 0 14px 29px rgba(15,23,42,.08);
    min-height: 120px;
}
.kpi-icon {
    width: 55px;
...
.rubric {
    ...
    color:#92400e;
    margin:.9rem 0;
    box-shadow: 0 8px 18px rgba(245,158,11,.09);
}
.warn {
    padding:.9rem 1rem;
    border-radius:18px;
...
PLACE_NAMES = {
    "Housing": [
        "Philadelphia County Housing Stress Profile",
        "Orange County Rent Burden Profile",
        "Cook County Housing Pressure Profile",
        "Miami-Dade County Affordability Profile",
        "Allegheny County Rental Stress Profile",
        "Los Angeles County Housing Burden Profile",
        "Maricopa County Housing Pressure Profile",
        "Harris County Rent Burden Profile"
    ],
    "Debt": [
        "Cook County Household Debt Profile",
        "Philadelphia County Debt Pressure Profile",
        "Orange County Consumer Debt Profile",
        "Miami-Dade County Credit Stress Profile",
        "Dallas County Debt Vulnerability Profile",
        "Broward County Household Debt Profile",
        "Wayne County Credit Risk Profile",
        "Clark County Debt Pressure Profile"
    ],
    "Cost": [
        "Philadelphia County Cost Pressure Profile",
        "Cook County Cost-of-Living Profile",
        "Orange County Basic Needs Profile",
        "Miami-Dade County Living Cost Profile",
        "Fulton County Income Stress Profile",
        "King County Cost Pressure Profile",
        "Riverside County Living Cost Profile",
        "Queens County Cost Burden Profile"
    ],
    "Energy": [
        "Allegheny County Energy Burden Profile",
        "Cook County Utility Cost Profile",
        "Philadelphia County Energy Stress Profile",
        "Wayne County Heating Burden Profile",
        "Cuyahoga County Utility Burden Profile",
        "Maricopa County Energy Cost Profile",
        "Harris County Utility Stress Profile",
        "Erie County Energy Burden Profile"
    ],
    "Food": [
        "Philadelphia County Food Access Profile",
        "Cook County Grocery Access Profile",
        "Orange County Food Access Profile",
        "Miami-Dade County Low-Access Food Profile",
        "Fresno County Grocery Distance Profile",
        "Wayne County Food Access Profile",
        "Cuyahoga County Nutrition Access Profile",
        "Maricopa County Food Access Profile"
    ]
}
...
def make_profile_name(row):
    ...
    prefix = {"High": "High-Risk", "Medium": "Moderate-Risk", "Low": "Lower-Risk"}.get(risk, "Risk")
...
def debtshield_chatbot(question, row, selected):
    """A lightweight, explainable chatbot that answers from the selected profile only."""
    ...
    top_three = ", ".join([f"{name} ({score:.1f})" for name, score in ranked[:3]])

    if not q:
        return "Ask me about this profile's risk level, strongest drivers, or recommended interventions."

    if any(word in q for word in ["hello", "hi", "hey"]):
        return (f"Hello! I’m the DebtShield Assistant. I can explain **{selected}**, including its "
                f"risk level, strongest drivers, and recommended actions.")

    if any(word in q for word in ["summary", "summarize", "overview", "explain this profile"]):
        return (f"**{selected}** has a **{risk}** risk classification and a Financial Distress Index of **{index:.1f}**. "
                f"Its strongest driver is **{driver} ({driver_score:.1f})**. The three highest pressures are {top_three}.")

    if any(word in q for word in ["why", "driver", "cause", "risk factor", "highest"]):
        return (f"The strongest risk driver is **{driver}**, with a score of **{driver_score:.1f}**. "
                f"The next highest factors are **{ranked[1][0]} ({ranked[1][1]:.1f})** and "
                f"**{ranked[2][0]} ({ranked[2][1]:.1f})**. These scores are what most strongly shape the profile's overall distress level.")

    if any(word in q for word in ["recommend", "action", "solution", "priority", "intervention", "help"]):
        items = recs(row)
        actions = " ".join([f"**{title}:** {text}" for title, text in items])
        return (f"The first priority should be **{driver}**, because it has the highest stress score. {actions}")

    if any(word in q for word in ["housing", "rent", "eviction"]):
        score = scores["Housing"]
        return (f"Housing stress is **{score:.1f}**. Recommended responses include rental assistance, eviction diversion, "
                f"housing counseling, and expansion of affordable housing options.")

    if any(word in q for word in ["debt", "credit", "delinquency", "loan"]):
        score = scores["Debt"]
        return (f"Debt stress is **{score:.1f}**. Recommended responses include debt counseling, delinquency outreach, "
                f"credit restructuring, and short-term emergency cash support.")

    if any(word in q for word in ["cost", "income", "job", "wage", "poverty"]):
        score = scores["Cost of Living"]
        return (f"Cost-of-living pressure is **{score:.1f}**. Useful interventions include benefits enrollment, job placement, "
                f"wage support, and targeted basic-needs assistance.")

    if any(word in q for word in ["energy", "utility", "electric", "heating"]):
        score = scores["Energy"]
        return (f"Energy stress is **{score:.1f}**. Recommended responses include utility assistance, weatherization, "
                f"energy-efficiency upgrades, and bill-payment support.")

    if any(word in q for word in ["food", "grocery", "snap", "nutrition"]):
        score = scores["Food Access"]
        return (f"Food-access risk is **{score:.1f}**. Recommended responses include SNAP outreach, mobile markets, "
                f"transportation support, and improved access to full-service grocery retailers.")

    if any(word in q for word in ["low", "medium", "high", "classification", "score", "index"]):
        return (f"This profile is classified as **{risk} risk** with a Financial Distress Index of **{index:.1f}**. "
                f"Its leading issue is **{driver} ({driver_score:.1f})**.")

    return (f"For **{selected}**, the main issue is **{driver} ({driver_score:.1f})**, and the overall risk level is **{risk}**. "
            f"Try asking: “Why is this profile at risk?”, “What should policymakers prioritize?”, or “Explain the housing score.”")
...
<div class="sidebar-title">
  ...
  <div>
    <div class="sidebar-name">DebtShield AI</div>
  </div>
</div>
<div class="sidebar-subtitle">Financial vulnerability intelligence platform</div>
""", unsafe_allow_html=True)

uploaded = st.sidebar.file_uploader("Upload a different CSV dataset (optional)", type=["csv"])

DEFAULT_CSV = "week1_cleaned_financial_distress_dataset.csv"
...
if uploaded is not None:
    ...
    source = "Built-in Week 1 dataset"
else:
    raw = pd.DataFrame([{
...
page = st.sidebar.radio(
    "Navigation",
    ["🏠 Executive Dashboard", "👥 Community Profile", "📊 Risk Drivers", "📈 Model Analytics", "🧭 Recommendations", "ℹ️ About"]
)
...
<div class="hero">
  <div class="hero-content">
    ...
    <span class="badge">🗄️ 11 PUBLIC DATA SOURCES</span>
    <span class="badge">🧠 EXPLAINABLE RISK ENGINE</span>
    <span class="badge">🏛️ POLICY RECOMMENDATIONS</span>
    <div class="hero-main">
      <div class="hero-logo">🛡️</div>
      <div class="hero-title">DebtShield AI</div>
    </div>
    <div class="hero-sub">
...
if page == "🏠 Executive Dashboard":
    ...
    st.markdown('<div class="rubric"><b>⚖️ Rubric of risk & financial distress:</b> These two charts represent dataset-level distributions, not individual profiles. They are designed to remain consistent unless the underlying dataset changes. Individual community metrics, risk drivers, and recommendations will change when you select a different profile.</div>', unsafe_allow_html=True)
    ...
    st.markdown(
        '<div class="note"><b>Profile assistant:</b> Ask simple questions about the selected community’s risk level, strongest drivers, and recommended interventions. Responses are generated directly from the displayed profile data.</div>',
        unsafe_allow_html=True
    )
    ...
    if chat_key not in st.session_state:
        st.session_state[chat_key] = [{
            "role": "assistant",
            "content": f"I’m ready to explain **{selected}**. Ask me about its risk level, main drivers, or policy recommendations."
        }]

    q1, q2, q3 = st.columns(3)
    quick_prompt = None
    if q1.button("Why is this profile at risk?", use_container_width=True):
    ...
    if q3.button("Give me a short summary", use_container_width=True):
        quick_prompt = "Give me a short summary of this profile."

    for message in st.session_state[chat_key]:
        with st.chat_message(message["role"]):
            st.markdown(message["content"])

    typed_prompt = st.chat_input("Ask DebtShield about this profile...")
    ...
    if prompt:
        ...
        answer = debtshield_chatbot(prompt, row, selected)
        st.session_state[chat_key].append({"role": "assistant", "content": answer})
        with st.chat_message("assistant"):
            st.markdown(answer)

    st.caption("DebtShield Assistant is a rule-based research prototype. It explains the selected dataset profile and does not provide personal financial advice.")
    ...
    st.markdown("""
    <div class="info-card">
    <b>DebtShield AI</b> integrates housing, debt, labor, food access, energy burden, and social vulnerability indicators into one financial risk platform.
    It generates a Financial Distress Index, explains the strongest risk drivers, and produces policy recommendations.
    </div>
    """, unsafe_allow_html=True)

    st.markdown("### How to read the community names")
    st.markdown("""
    <div class="info-card">
    The current version uses synthetic demonstration profiles, but the labels are written like realistic county profiles
    such as <b>Philadelphia County Housing Stress Profile</b>, <b>Cook County Debt Pressure Profile</b>, or
    <b>Orange County Food Access Profile</b>. These labels make the app easier to understand while the next version is expanded to fully real county-level data.
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("""
    <div class="info-card">
    ACS, Zillow, Eviction Lab, Federal Reserve SCF, BLS, MIT Living Wage, CDC SVI, FHFA, USDA Food Access,
    DOE LEAD, and NY Fed Consumer Credit.
    </div>
    """, unsafe_allow_html=True)
    ...
    <ul>
      <li>Replace synthetic rows with real county-level observations</li>
      <li>Add Pennsylvania and U.S. risk maps</li>
      <li>Add a future-risk simulator</li>
      <li>Add a SHAP explanation panel inside the app</li>
    </ul>
    </div>
    """, unsafe_allow_html=True)

st.markdown('<div class="footer">DebtShield AI v9 · Financial vulnerability intelligence platform · Profile assistant enabled · Prototype for research and demonstration.</div>', unsafe_allow_html=True)
